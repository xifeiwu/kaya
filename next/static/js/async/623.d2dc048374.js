...
var best_index : i32 = 0;`,`if (${e.getByIndices("input_indices")} ${r.selectLastIndex>0?">=":">"} value) {
    ...
    `,r_=(e,r,i,a,n,s,o,u,l,d,p,c)=>{var h,f,m,g,y,_,b,$,v,w,x,k,S,T,I,E,z,C,A,O,R,B,N,M,D;let P,U,q,W,L,V,G,H,F,j,K,Q,Z,X,Y,J,ee,et,er,ei,ea,en,es,eo,eu,el,ed,ep,ec,eh,ef,em,eg,ey=Math.min(e.outputCount,1+ +!!o+ +!!u),e_=ey>1?d.pastSequenceLength:0,eb=e_+d.kvSequenceLength,e$=l&&e8.size(l.dims)>0?l:void 0,ev=[r,i];ey>1&&o&&e8.size(o.dims)>0&&ev.push(o),e$&&ev.push(e$),p&&ev.push(p),c&&ev.push(c);let ew=e.compute((h=ey,f=r,m=i,g=o,y=e$,_=d,b=e_,$=p,v=c,P=b+_.kvSequenceLength,U=[_.batchSize,_.numHeads,_.sequenceLength,P],q=h>1&&g,W=_.kvNumHeads?_.kvNumHeads:_.numHeads,L=q?[_.batchSize,W,P,_.headSize]:void 0,V=_.nReps?_.nReps:1,G=0===_.scale?1/Math.sqrt(_.headSize):_.scale,H=tC(_.headSize),F=_.headSize/H,j={x:Math.ceil(P/12),y:Math.ceil(_.sequenceLength/12),z:_.batchSize*_.numHeads},K=[{type:12,data:_.sequenceLength},{type:12,data:F},{type:12,data:P},{type:12,data:_.numHeads},{type:12,data:_.headSize},{type:1,data:G},{type:12,data:b},{type:12,data:_.kvSequenceLength},{type:12,data:V}],Q=q&&g&&e8.size(g.dims)>0,Z=["type","type"],Q&&Z.push("type"),y&&Z.push("type"),$&&Z.push("type"),v&&Z.push("type"),X=[{dims:U,dataType:f.dataType,gpuDataType:0}],q&&X.push({dims:L,dataType:f.dataType,gpuDataType:0}),{name:"AttentionProbs",shaderCache:{hint:`${H};${void 0!==y};${void 0!==g};${h}`,inputDependencies:Z},getRunData:()=>({outputs:X,dispatchGroup:j,programUniforms:K}),getShaderSource:e=>{let r=tM("q",f.dataType,f.dims,H),i=[r,tM("key",m.dataType,m.dims,H)];if(Q){let e=tM("past_key",g.dataType,g.dims,H);i.push(e)}y&&i.push(tM("attention_bias",y.dataType,y.dims));let a=$?tM("seq_lens",$.dataType,$.dims):void 0;a&&i.push(a);let n=v?tM("total_sequence_length_input",v.dataType,v.dims):void 0;n&&i.push(n);let s=tD("output",f.dataType,U),o=[s];q&&o.push(tD("present_key",f.dataType,L,H));let u=tE(1,H);return`
  const TILE_SIZE = 12u;
  ...
  }`},getRunData:()=>({outputs:[],dispatchGroup:{x:1,y:T,z:x*k},programUniforms:et})}),{inputs:p&&c?[ew,p,c]:[ew],outputs:[]});let ex=[ew,a];ey>1&&u&&e8.size(u.dims)>0&&ex.push(u),p&&ex.push(p),c&&ex.push(c),e.compute((C=ey,A=ew,O=a,R=u,B=d,N=e_,M=p,D=c,en=N+B.kvSequenceLength,es=B.nReps?B.nReps:1,eo=B.vHiddenSize*es,eu=C>1&&R,el=B.kvNumHeads?B.kvNumHeads:B.numHeads,ed=eu?[B.batchSize,el,en,B.headSize]:void 0,ep=[B.batchSize,B.sequenceLength,eo],ec={x:Math.ceil(B.vHeadSize/12),y:Math.ceil(B.sequenceLength/12),z:B.batchSize*B.numHeads},eh=[{type:12,data:B.sequenceLength},{type:12,data:en},{type:12,data:B.vHeadSize},{type:12,data:B.numHeads},{type:12,data:B.headSize},{type:12,data:eo},{type:12,data:N},{type:12,data:B.kvSequenceLength},{type:12,data:es}],ef=eu&&R&&e8.size(R.dims)>0,em=["type","type"],ef&&em.push("type"),M&&em.push("type"),D&&em.push("type"),eg=[{dims:ep,dataType:A.dataType,gpuDataType:0}],eu&&eg.push({dims:ed,dataType:A.dataType,gpuDataType:0}),{name:"AttentionScore",shaderCache:{hint:`${void 0!==R};${C}`,inputDependencies:em},getRunData:()=>({outputs:eg,dispatchGroup:ec,programUniforms:eh}),getShaderSource:e=>{let r=tM("probs",A.dataType,A.dims),i=[r,tM("v",O.dataType,O.dims)];ef&&i.push(tM("past_value",R.dataType,R.dims));let a=M?tM("seq_lens",M.dataType,M.dims):void 0;M&&i.push(a);let n=D?tM("total_sequence_length_input",D.dataType,D.dims):void 0;D&&i.push(n);let s=[tD("output",A.dataType,ep)];return eu&&s.push(tD("present_value",A.dataType,ed)),`
  const TILE_SIZE = 12u;
...
}`,rU=e=>{let r=tE(e.inputs[0].dataType);e.compute(rw(e.inputs[0],"Erf",e=>`erf_vf32(${e})`,rP(r)))},rq=e=>{e.compute(rw(e.inputs[0],"Exp","exp"))},rW=e=>{e.compute(rw(e.inputs[0],"Floor","floor"))},rL=e=>{let r=tE(e.inputs[0].dataType);e.compute(rw(e.inputs[0],"Gelu",e=>`0.5 * ${e} * (1.0 + erf_vf32(${e} * 0.7071067811865475))`,rP(r)))},rV=(e,r)=>{let i=tE(e.inputs[0].dataType);e.compute(rw(e.inputs[0],"LeakyRelu",e=>`select(leaky_relu_alpha_ * ${e}, ${e}, ${e} >= vec4<${i}>(0.0))`,`const leaky_relu_alpha_ = ${i}(${r.alpha});`,r.cacheKey))},rG=e=>{e.compute(rw(e.inputs[0],"Not",e=>`!${e}`))},rH=e=>{e.compute(rw(e.inputs[0],"Neg",e=>`-${e}`))},rF=e=>{e.compute(rw(e.inputs[0],"Reciprocal",e=>`1.0/${e}`))},rj=e=>{let r=tE(e.inputs[0].dataType);e.compute(rw(e.inputs[0],"Relu",e=>`select(vec4<${r}>(0.0), ${e}, ${e} > vec4<${r}>(0.0))`))},rK=e=>{e.compute(rw(e.inputs[0],"Sigmoid",e=>`(1.0 / (1.0 + exp(-${e})))`))},rQ=e=>tk(e),rZ=(e,r)=>{let i=tE(e.inputs[0].dataType);e.compute(rw(e.inputs[0],"HardSigmoid",e=>`max(vec4<${i}>(0.0), min(vec4<${i}>(1.0), ${r.alpha} * ${e} + vec4<${i}>(${r.beta})))`,void 0,r.cacheKey))},rX=e=>{e.compute(rw(e.inputs[0],"Sin","sin"))},rY=e=>{e.compute(rw(e.inputs[0],"Sinh","sinh"))},rJ=e=>{e.compute(rw(e.inputs[0],"Sqrt","sqrt"))},r0=e=>{e.compute(rw(e.inputs[0],"Tan","tan"))},r1=e=>`sign(${e}) * (1 - exp(-2 * abs(${e}))) / (1 + exp(-2 * abs(${e})))`,r2=e=>{e.compute(rw(e.inputs[0],"Tanh",r1))},r3=(e="f32")=>`
...
`)}
  ...
  ${r.mainStart()}
    ...
    ${_?"inv_std_output[global_idx] = inv_std_dev":""};
  }`}}})(e.inputs,r,e.outputCount))}}),si=V(()=>{n$(),nL(),nV(),aR=e=>{var r=e.inputs;if(!r||2!==r.length)throw Error("MatMul requires 2 inputs.");if(r[0].dims[r[0].dims.length-1]!==r[1].dims[r[1].dims.length-2])throw Error("shared dimension does not match.");let i=e6.calcShape(e.inputs[0].dims,e.inputs[1].dims,!0);if(!i)throw Error("Can't use matmul on the given tensors");let a=i[i.length-1],n=e.inputs[0].dims[e.inputs[0].dims.length-1];if(a<8&&n<8)e.compute(ik(e.inputs,{activation:""},i));else{let r=i[i.length-2],s=e8.size(e.inputs[0].dims.slice(0,-2)),o=e8.size(e.inputs[1].dims.slice(0,-2));if(1!==s&&1===r&&1===o){let r=e.inputs[0].reshape([1,s,n]),o=e.inputs[1].reshape([1,n,a]),u=[1,s,a],l=[r,o];e.compute(iE(l,{activation:""},i,u),{inputs:l})}else e.compute(iE(e.inputs,{activation:""},i))}}}),sa=V(()=>{ny(),n$(),nT(),nI(),aB=(e,r)=>{var i,a,n,s;let o,u,l,d,p,c,h,f,m,g,y,_,b,$,v,w,x,k,S,T,I,E,z,C,A,O,R,B,N,M,D,P,U,q,W,L,V,G,H,F,j;((e,r)=>{if(e.length<3||e.length>4)throw Error("MatMulNBits requires 3 or 4 inputs");let i=e[0],a=i.dims.length;if(i.dims[a-1]!==r.k)throw Error("The last dim of input shape does not match the k value");let n=Math.floor((r.k+r.blockSize-1)/r.blockSize),s=r.blockSize/8*r.bits,o=e[1];if(!e8.areEqual(o.dims,[r.n,n,s]))throw Error("The second inputs must be 3D tensor with shape N X nBlocksPerCol X blobSize");let u=e[2].dims;if(e8.size(u)!==r.n*n)throw Error("scales input size error.");if(4===e.length){let i=e[3].dims,a=r.n*(8===r.bits?n:Math.floor((n*r.bits+7)/8));if(e8.size(i)!==a)throw Error("zeroPoints input size error.")}})(e.inputs,r),32===r.blockSize&&e.adapterInfo.isVendor("intel")&&e.adapterInfo.isArchitecture("gen-12lp")?e.compute((i=e.inputs,a=r,u=(o=i[0].dims).length,l=o[u-2],d=a.k,p=a.n,c=o.slice(0,u-2),h=e8.size(c),f=i[1].dims[2]/4,m=i[0].dataType,g=tC(a.k),y=tC(f),_=c.concat([l,p]),w=(v=($=128/(b=p%8==0?8:p%4==0?4:1))*y*8)/g,x=v/a.blockSize,k=e8.size(_)/b,S=[],T=[h,l,d/g],(I=e8.convertShape(i[1].dims).slice()).splice(-1,1,f/y),S.push(...tz(T)),S.push(...tz(I)),S.push(...tz(i[2].dims)),4===i.length&&S.push(...tz(e8.convertShape(i[3].dims))),E=[h,l,p],S.push(...tz(E)),{name:"BlockwiseMatMulNBits32",shaderCache:{hint:`${a.blockSize};${g};${y};${$};${b}`,inputDependencies:Array(i.length).fill("rank")},getRunData:()=>({outputs:[{dims:_,dataType:m}],dispatchGroup:{x:k},programUniforms:S}),getShaderSource:e=>{let r=T.length,n=tM("a",i[0].dataType,r,g),s=tM("b",12,I.length,y),o=tM("scales",i[2].dataType,i[2].dims.length),u=[n,s,o],l=4===i.length?tM("zero_points",12,i[3].dims.length):void 0;l&&u.push(l);let d=E.length,p=tD("output",i[0].dataType,d),c=tI(i[0].dataType);return`
        ...
        }`}})):e.compute((n=e.inputs,s=r,C=(z=n[0].dims).length,A=z[C-2],O=s.k,R=s.n,B=z.slice(0,C-2),N=e8.size(B),M=n[1].dims[2]/4,D=n[0].dataType,P=tC(s.k),U=tC(M),q=tC(R),W=B.concat([A,R]),L=A>1&&R/q%2==0?2:1,V=e8.size(W)/q/L,G=[],H=[N,A,O/P],(F=e8.convertShape(n[1].dims).slice()).splice(-1,1,M/U),G.push(...tz(H)),G.push(...tz(F)),G.push(...tz(n[2].dims)),4===n.length&&G.push(...tz(e8.convertShape(n[3].dims))),j=[N,A,R/q],G.push(...tz(j)),{name:"MatMulNBits",shaderCache:{hint:`${s.blockSize};${s.bits};${P};${U};${q};${L};64`,inputDependencies:Array(n.length).fill("rank")},getRunData:()=>({outputs:[{dims:W,dataType:D}],dispatchGroup:{x:V},programUniforms:G}),getShaderSource:e=>{let r=H.length,i=tM("a",n[0].dataType,r,P),a=tM("b",12,F.length,U),o=tM("scales",n[2].dataType,n[2].dims.length),u=[i,a,o],l=4===n.length?tM("zero_points",12,n[3].dims.length):void 0;l&&u.push(l);let d=j.length,p=tD("output",n[0].dataType,d,q),c=tI(n[0].dataType),h=(()=>{switch(P){case 1:return`array<${c}, 8>`;case 2:return`mat4x2<${c}>`;case 4:return`mat2x4<${c}>`;default:throw Error(`${P}-component is not supported.`)}})();return`
